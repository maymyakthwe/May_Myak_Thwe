...
const Images = ({img,imgIndex}:{img:string[],imgIndex:number}) => {
  return (
    <> {img.map((imgSrc,idx)=>{
        ...
        className='aspect-video w-full shrink-0 rounded-xl bg-neutral-800 object-cover border-2 border-white shadow-[5px_6px_20px_rgba(99,102,241,0.4)] '></motion.div>
        })}
    </>
  )
}
